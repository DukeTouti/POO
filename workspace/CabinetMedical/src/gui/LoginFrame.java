...
//LoginFrame EST une fenêtre. 
//Elle possede toutes les capacites d'une fenêtre Windows classique.
public class LoginFrame extends JFrame {
	...
	// On definit les styles ici en utilisant des constantes globales. (static
	// final)
	...
	public LoginFrame() {
		...
		// GridBagConstraints sert a positionner et organiser les 3 elements du
		// formulaire de connexion
		// Comme une "regle/objet de configuration" on ecrase les reglages standards
		// (centre, pas d'etirement, pas de marge).
		GridBagConstraints gbc = new GridBagConstraints();// permet de definir comment chaque composant doit être
															// positionne
		gbc.insets = new Insets(15, 0, 15, 0);// Espacement vertical de 15px en haut ains qu'en bas (padding -> (Top,
												// Left, Bottom, Right)
		...
		JButton btnLogin = new JButton("ACCEDER") {// On modifie le "pinceau" ou facon de draw le bouton
			...
			protected void paintComponent(Graphics g) {
				// Graphics2D est la version plus recente de l'outil de "dessin" on "cast" en
				// quelque sotte
				...
				g2.fillRoundRect(0, 0, getWidth() - 1, getHeight() - 1, 20, 20); // dessines un rectangle avec des coins
																					// arrondis
				g2.dispose();// On ferme la configuration du "Pinceau"
				// On "passe la main" au pinceau standard JButton
				super.paintComponent(g);// Aapres la finalisation on laisse la main pour que le systeme puisse ecrire
										// par dessus
			}
		};
	...
	private void appliquerStyle(JTextField field, String titre) {
		...
		field.setBorder(BorderFactory.createTitledBorder(new RoundedBorder(Color.WHITE, 20), titre, // Texte a
																									// afficher..
